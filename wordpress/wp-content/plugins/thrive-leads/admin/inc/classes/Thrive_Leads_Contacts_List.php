...
class Thrive_Leads_Contacts_List extends WP_List_Table
{
    ...
    protected function column_actions($item)
    {
        ...
        $actions = array(
            'email' => sprintf('<a class="tve-email tve-icon-mail" href="javascript:void(0)" data-contact-id="%s" title="%s"></a>',
                $item->id,
                __('Email', "thrive-leads")
            ),
            'delete' => sprintf('<a class="tve-icon-trash-o" href="?page=thrive_leads_contacts&action=%s&contact=%s&paged=%s&_wpnonce=%s" title="%s"></a>',
                'delete',
                $item->id,
                $this->get_pagenum(),
                $delete_nonce,
