 ...
 * Time: 18:23
 */
class Thrive_List_Connection_ReCaptcha extends Thrive_List_Connection_Abstract
{
    /**
    ...
    public function readCredentials()
    {
        ...
        if (empty($site) || empty($secret)) {
            return $this->error(__('Both Site Key and Secret Key fields are required', 'thrive-cb'));
        }
        ...
        if ($result !== true) {
            return $this->error(sprintf(__('Incorrect Secret Key.', 'thrive-cb'), $result));
        }
         ...
         */
        $this->save();
        $this->success(__('ReCaptcha connected successfully!', 'thrive-cb'));
    }
