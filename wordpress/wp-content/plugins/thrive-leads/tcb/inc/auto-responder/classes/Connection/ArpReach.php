 ...
 * Time: 4:59 PM
 */
class Thrive_List_Connection_ArpReach extends Thrive_List_Connection_Abstract
{
    /**
    ...
    public function readCredentials()
    {
        ...
        if (empty($url) || empty($app_key)) {
            return $this->error(__("Invalid URL or API key", "thrive-cb"));
        }

        if (empty($lists)) {
            return $this->error(__('Please provide at least one list for your subscribers', 'thrive-cb'));
        }
        ...
        $this->success(__('ArpReach connected successfully', 'thrive-cb'));
    }
    ...
    {
        $api = new Thrive_Api_ArpReach($this->param('url'), $this->param('api_key'));
