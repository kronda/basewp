...
class Thrive_Api_iContact
{
     ...
     * @static
     * @access public
     * @param Thrive_Api_iContact $oInstance Instance to set to
     * @return Thrive_Api_iContact $this
     **/
    ...
    public function makeCall($sResource, $sMethod = 'GET', $mPostData = null, $sReturnKey = null)
    {
        ...
        foreach ($aRequiredConfigs as $sKey) {
            ...
            if (empty($this->aConfig[$sKey])) {
                throw new Thrive_Api_iContact_Exception("{$sKey} is undefined.");
            }
        }
        ...
        switch ($sMethod) {
            ...
            case 'POST':
                // Check for POST data
                if (empty($mPostData)) {
                    throw new Thrive_Api_iContact_Exception('No POST data was provided.');
                }
            ...
            case 'PUT':
                ...
                $args['method'] = 'PUT';
                if (empty($mPostData)) {
                    throw new Thrive_Api_iContact_Exception('No file or data specified for PUT request');
                }
                if (!is_string($mPostData) || !file_exists($mPostData)) {
                    ...
                    if ($rFileContentHandle === false) {
                        throw new Thrive_Api_iContact_Exception('A non-existant file was specified for POST data, or the file could not be opened.');
                    }
                ...
                break;
            default:
                throw new Thrive_Api_iContact_Exception('Invalid method: ' . $sMethod);
        }
        ...
        if ($result instanceof WP_Error) {
            throw new Thrive_Api_iContact_Exception($result->get_error_message());
        }
        ...
        if (empty($aResponse)) {
            throw new Thrive_Api_iContact_Exception('iContact API did not return a valid JSON');
        }
        ...
        if (!empty($aResponse->errors)) {
            ...
            foreach ($aResponse->errors as $sError) {
                throw new Thrive_Api_iContact_Exception($sError);
            }
        }
    ...
    public function subscribeContactToList($iContactId, $iListId, $sStatus = 'normal')
    {
        ...
        if (empty($aSubscriptions->subscriptions) && !empty($aSubscriptions->warnings)) {
            throw new Thrive_Api_iContact_Exception($aSubscriptions->warnings[0]);
        }
