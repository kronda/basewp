...
class Thrive_Api_MadMimi
{
     ...
     *
     * @throws Thrive_Api_MadMimi_Exception
     */
    public function __construct($apiKey, $username)
    {
        if (empty($apiKey)) {
            throw new Thrive_Api_MadMimi_Exception('API Key is required');
        }
        $this->apiKey = $apiKey;
    ...
    public function getAudienceLists()
    {
        if (empty($this->username) || empty($this->apiKey)) {
            throw new Thrive_Api_MadMimi_Exception('Illegal Arguments');
        }
    ...
    protected function _call($path, $params = array(), $method = 'GET')
    {
        ...
        if ($result instanceof WP_Error) {
            throw new Thrive_Api_MadMimi_Exception('Failed connecting to MadMimi: ' . $result->get_error_message());
        }
        ...
        if ($statusMsg != 'OK') {
            throw new Thrive_Api_MadMimi_Exception('API call error: ' . $statusMsg);
        }
