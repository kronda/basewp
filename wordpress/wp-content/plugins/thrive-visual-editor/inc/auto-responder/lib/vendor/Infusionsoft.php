...
class Thrive_Api_Infusionsoft
{
    ...
    public function __construct($subdomain = NULL, $api_key = NULL)
    {
        ...
        if (empty($this->subdomain) || empty($this->api_key)) {
            throw new Thrive_Api_Infusionsoft_InfusionsoftException("You must provide a ClientID and API key for your Infusionsoft application.");
        }
    }
    ...
    public function __call($name, $arguments)
    {
        ...
        if (!call_user_func_array(array($client, 'query'), $arguments)) {
            throw new Thrive_Api_Infusionsoft_InfusionsoftException($client->getErrorMessage());
        }
