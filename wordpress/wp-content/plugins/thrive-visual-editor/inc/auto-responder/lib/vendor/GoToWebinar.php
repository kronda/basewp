...
class Thrive_Api_GoToWebinar
{
     ...
     *
     * @throws Thrive_Api_GoToWebinar_Exception
     */
    public function __construct($apiKey, $accessToken = null, $organizerKey = null)
    {
        if (empty($apiKey)) {
            throw new Thrive_Api_GoToWebinar_Exception('API Key is required');
        }
        $this->apiKey = $apiKey;
    ...
    protected function _call($path, $params = array(), $method = 'GET', $auth = true, $content_type = 'application/json')
    {
        ...
        switch ($method) {
            ...
            default:
                ...
                $result = thrive_api_remote_get($url, $args);
                break;
        }

        if ($result instanceof WP_Error) {
            throw new Thrive_Api_GoToWebinar_Exception('Failed connecting to GoToWebinar: ' . $result->get_error_message());
        }
        ...
        if (!empty($data['int_err_code'])) {
            throw new Thrive_Api_GoToWebinar_Exception('API call error: ' . $data['int_err_code']);
        }

        if (!empty($data['errorCode'])) {
            throw new Thrive_Api_GoToWebinar_Exception('API call error: ' . $data['errorCode'] . ( !empty($data['description']) ? "Error description: " . $data["description"] : ''));
        }

        if (!empty($data['err'])) {
            if (!empty($data['message'])) {
                throw new Thrive_Api_GoToWebinar_Exception('API call error: ' . $data["message"]);
            } else {
                throw new Thrive_Api_GoToWebinar_Exception('API call error: ' . var_export($data, true));
            }
