...
class Thrive_Api_AWeber_Base
{
     ...
     * @access public
     * @return Thrive_Api_AWeber_Entry or Thrive_Api_AWeber_Collection
     */
    ...
    protected function readResponse($response, $url)
    {
        ...
        if (!empty($response['id'])) {
            return new Thrive_Api_AWeber_Entry($response, $url, $this->adapter);
        } else if (array_key_exists('entries', $response)) {
            return new Thrive_Api_AWeber_Collection($response, $url, $this->adapter);
        }
        return false;
    }
} 