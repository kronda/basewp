...
class Thrive_Api_Sendreach
{
    ...
    /**
     * Thrive_Api_Sendreach constructor.
     */
    ...
    protected function _send($params = array())
    {
        ...
        $response = thrive_api_remote_get($this->api_url . '?' . http_build_query($getParams, '', '&'), $args);

        if (is_wp_error($response) || $response['response']['code'] != 200) {
            throw new Thrive_Api_Sendreach_Exception($response['response']['message']);
        }

        $body = json_decode($response['body']);

        if (is_object($body) && $body->code != 200) {
            throw new Thrive_Api_Sendreach_Exception($body->message);
        }
