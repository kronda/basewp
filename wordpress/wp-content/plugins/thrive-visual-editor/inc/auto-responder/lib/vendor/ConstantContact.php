 ...
 * Time: 10:30 AM
 */
class Thrive_Api_ConstantContact
{
    ...
    /**
     * Thrive_Api_ConstantContact constructor.
     */
    ...
    public function addContact($list_id, $user)
    {
        ...
        } catch (Thrive_Api_ConstantContact_AlreadyExistException $e) {
     ...
     * @param string $type
     * @return mixed
     * @throws Thrive_Api_ConstantContact_Exception
     * @throws Thrive_Api_ConstantContact_AlreadyExistException
     */
    protected function make_request($path, $params = array(), $type = 'post', $extra = array())
    {
        ...
        if ($response instanceof WP_Error) {
            throw new Thrive_Api_ConstantContact_Exception($response->get_error_message());
        }

        $httpCode = $response['response']['code'];

        if ($httpCode == '409') {
            throw new Thrive_Api_ConstantContact_AlreadyExistException('Info already exists');
        }

        if (!($httpCode == '200' || $httpCode == '201' || $httpCode == '204')) {
            throw new Thrive_Api_ConstantContact_Exception('API call failed. Server returned status code ' . $httpCode);
        }
