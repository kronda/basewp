...
class Thrive_List_Connection_ConstantContact extends Thrive_List_Connection_Abstract
{
    ...
    public function readCredentials()
    {
        ...
        if (empty($api_key) || empty($api_token)) {
            return $this->error(__('You must provide a valid Constant Contact API Key and API token', 'thrive-cb'));
        }
        ...
        if ($result !== true) {
            return $this->error(sprintf(__('Could not connect to Constant Contact using the provided API Key and API Token (<strong>%s</strong>)', 'thrive-cb'), $result));
        }
         ...
         */
        $this->save();
        $this->success(__('Constant Contact connected successfully', 'thrive-cb'));
    }
    ...
    {
        try {
            /** @var Thrive_Api_ConstantContact $api */
            $api = $this->getApi();
    ...
    {
        return new Thrive_Api_ConstantContact($this->param('api_key'), $this->param('api_token'));
    }
