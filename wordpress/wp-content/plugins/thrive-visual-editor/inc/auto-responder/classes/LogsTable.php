...
class Thrive_List_LogsTable extends WP_List_Table
{
    ...
    {
        $columns = array(
            'api_data' => __('Form Data', "thrive-cb"),
            'connection' => __('Service', "thrive-cb"),
            'date' => __('Date', "thrive-cb"),
            'error_message' => __('Error Message', "thrive-cb"),
            'actions' => 'Actions',
        );
    ...
    protected function column_api_data($item)
    {
        ...
        if (!empty($data['email'])) {
            $info .= "<strong>" . __("Email", "thrive-cb") . "</strong>: {$data['email']}<br/>";
        }

        if (!empty($data['name'])) {
            $info .= " <strong>" . __("Name", "thrive-cb") . "</strong>: {$data['name']}<br/>";
        }

        if (!empty($data['phone'])) {
            $info .= " <strong>" . __("Phone", "thrive-cb") . "</strong>: {$data['phone']}";
        }
