...
{
    if (empty($_REQUEST['page']) || $_REQUEST['page'] != 'thrive_integrations_section' || empty($_REQUEST['api'])) {
        return;
    }
    ...
    if (!empty($_REQUEST['disconnect'])) {
        ...
        wp_redirect(admin_url('admin.php?page=thrive_integrations_section&integration=thrive_auto_responder'));
        exit();
    } else {
        $connection->readCredentials();
    }

    wp_redirect(admin_url('admin.php?page=thrive_integrations_section&integration=thrive_auto_responder' . (Thrive_List_Manager::$ADMIN_HAS_ERROR ? '&api=' . $_REQUEST['api'] : '')));
    exit();
}
...
function tve_api_admin_scripts($hook)
{
     ...
     */
    thrive_dashboard_enqueue_script('tve-api-admin-global', thrive_dashboard_url() . 'auto-responder/views/js/admin-global.js', array('jquery'));

    if (strpos($hook, 'thrive_api_error_log') !== false) {
        thrive_dashboard_enqueue_script('tve-api-admin-logs-list', thrive_dashboard_url() . 'auto-responder/views/js/admin-logs-list.js', array('jquery'));
        thrive_dashboard_enqueue_style('tve-list-api', thrive_dashboard_url() . 'auto-responder/views/css/admin.css');
    }

    if (strpos($hook, 'thrive_integrations_section') === false) {
        return;
    }
