 ...
 * @version $id$
 */
class Thrive_Api_AWeber extends Thrive_Api_AWeber_Base
{
    ...
    public static function getDataFromAweberID($string)
    {
        ...
        $aweber = new Thrive_Api_AWeber($consumerKey, $consumerSecret);
    ...
    {
        if (empty($adapter)) {
            $serviceProvider = new Thrive_Api_AWeber_ServiceProvider();
            $adapter = new Thrive_Api_AWeber_Oauth_Application($serviceProvider);
    ...
    {
        if ($token && $secret) {
            $user = new Thrive_Api_AWeber_Oauth_User();
