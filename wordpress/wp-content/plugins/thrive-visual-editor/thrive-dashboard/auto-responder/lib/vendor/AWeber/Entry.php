...
class Thrive_Api_AWeber_Entry extends Thrive_Api_AWeber_Response
{
    ...
    /**
     * @var array Holds Thrive_Api_AWeber_Collection objects already instantiated, keyed by
    ...
    public function attrs()
    {
        ...
        if (!empty(Thrive_Api_AWeber::$_collectionMap[$this->type])) {
            foreach (Thrive_Api_AWeber::$_collectionMap[$this->type] as $child) {
     ...
     *
     * @access public
     * @param Thrive_Api_AWeber_Entry(List) List to move Resource (this) too.
     * @param mixed $last_followup_message_number_sent
     * @return mixed Thrive_Api_AWeber_Entry(Resource) Resource created on List ($list)
    ...
    public function move($list, $last_followup_message_number_sent = NULL)
    {
        ...
        return new Thrive_Api_AWeber_Entry($resource_data, $url, $this->adapter);
    }
     ...
     * @access public
     * @throws Thrive_Api_AWeber_ResourceNotImplemented
     * @return mixed
     */
    public function __get($value)
    {
        ...
        if (!empty($this->data) && array_key_exists($value, $this->data)) {
            if (is_array($this->data[$value])) {
                $array = new Thrive_Api_AWeber_EntryDataArray($this->data[$value], $value, $this);
        ...
        throw new Thrive_Api_AWeber_ResourceNotImplemented($this, $value);
    }
     ...
     * @access public
     * @return Thrive_Api_AWeber_Collection
     */
    public function findSubscribers($search_data)
    {
        ...
        return new Thrive_Api_AWeber_Collection($data, $url, $this->adapter);
    }
    ...
    public function getParentEntry()
    {
        ...
        try {
            ...
            return new Thrive_Api_AWeber_Entry($data, $url, $this->adapter);
        } catch (Exception $e) {
            return NULL;
        }
    }
    ...
    protected function _parseNamedOperation($data)
    {
        ...
        foreach ($data as $entryData) {
            $results[] = new Thrive_Api_AWeber_Entry($entryData, str_replace($this->adapter->app->getBaseUri(), '',
    ...
    protected function _methodFor($entryTypes)
    {
        ...
        throw new Thrive_Api_AWeber_ResourceNotImplemented($this);
    }

    /**
     * _getCollection
     *
     * Returns the Thrive_Api_AWeber_Collection object representing the given
    ...
    protected function _getCollection($value)
    {
        if (empty($this->_collections[$value])) {
            $url = "{$this->url}/{$value}";
            $data = $this->adapter->request('GET', $url);
            $this->_collections[$value] = new Thrive_Api_AWeber_Collection($data, $url, $this->adapter);
        }
    ...
    protected function _isChildCollection($value)
    {
        $this->_type();
        if (!empty(Thrive_Api_AWeber::$_collectionMap[$this->type]) &&
            in_array($value, Thrive_Api_AWeber::$_collectionMap[$this->type])
        ) return true;
        return false;
    }
