...
class TCB_Landing_Page_Transfer
{
    ...
    public function exportIconPack(ZipArchive $zip, & $config)
    {
        ...
        $options = array(
            'add_path' => 'icon-pack/fonts/',
            'remove_all_path' => true
        );
        $zip->addGlob(trailingslashit($thrive_icon_pack['folder']) . 'fonts/*', GLOB_BRACE, $options);
    ...
    public function exportExtraIconPacks(ZipArchive $zip, & $config)
    {
        ...
        foreach ($config['tve_globals']['extra_icons'] as $i => $icon_pack) {
            ...
            $options = array(
                'add_path' => $icon_zip_folder . '/fonts/',
                'remove_all_path' => true
            );
            $zip->addGlob(trailingslashit($icon_pack['folder']) . 'fonts/*', GLOB_BRACE, $options);
    ...
    protected function exportAddFontToZip(ZipArchive $zip, $folder_path, $css_file, $zip_folder_name = null)
    {
        ...
        /* add all font files */
        $options = array(
            'add_path' => 'font-pack/' . $name . '/',
            'remove_all_path' => true
        );
        $zip->addGlob(trailingslashit($folder_path) . '*.{woff,WOFF,eot,EOT,woff2,WOFF2,ttf,TTF,svg,SVG}', GLOB_BRACE, $options);
    }
