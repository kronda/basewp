...
final class WPCF_Page_Edit_Termmeta_Form extends Types_Admin_Edit_Fields {
	...
	public function __construct() {
		...
		$this->type = WPCF_Field_Term_Definition_Factory::FIELD_DEFINITIONS_OPTION;
	...
	protected function save() {
		...
		if( ! wpcf_is_client() ) {
			wpcf_admin_message_store(
				apply_filters(
					'types_message_term_fields_saved',
					__('Group saved', 'wpcf'),
					0,
					isset( $_GET['group_id'] ) ? true : false
				),
				'custom'
			);
		}
			...
			if ( defined('ICL_SITEPRESS_VERSION') && version_compare ( ICL_SITEPRESS_VERSION, '3.2', '<' ) ) {
				if ( function_exists( 'wpml_cf_translation_preferences_store' ) ) {
					$real_custom_field_name = wpcf_types_get_meta_prefix(
						wpcf_admin_fields_get_field( $field_slug, false, false, false, WPCF_Field_Term_Definition_Factory::FIELD_DEFINITIONS_OPTION )
					) . $field_slug;
