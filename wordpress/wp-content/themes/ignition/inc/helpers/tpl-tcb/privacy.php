...
<p>When accessing the <?php echo $options["website"] ?>  website, <?php echo $options["company"] ?> will learn certain
    information
...
<p><?php echo $options["company"] ?> will not obtain personally-identifying information about you when you visit our
    site,
...
<p><?php echo $options["website"] ?> may contain links to many other websites. <?php echo $options["company"] ?> cannot
    guarantee
    the accuracy of information found at any linked site. Links to or from external sites not owned or controlled
    by <?php echo $options["company"] ?>
    do not constitute an endorsement by <?php echo $options["company"] ?> or any of its employees of
...
<p>All copyrights, trademarks, patents and other intellectual property rights in and on our website and all content and
    software located on the site shall remain the sole property of <?php echo $options["company"] ?> or its licensors.
    The use
    of our trademarks, content and intellectual property is forbidden without the express written consent
    from <?php echo $options["company"] ?>.</p>
...
<p><?php echo $options["company"] ?> is not responsible for any of the opinions or comments posted
    on <?php echo $options["website"]
    ?>. <?php echo $options["company"] ?> is not a forum for testimonials, however provides testimonials as a means for
    customers to share their experiences with one another. To protect against abuse, all testimonials appear after they
    have been reviewed by management of <?php echo $options["company"] ?>. <?php echo $options["company"] ?> doe not
    share the
    opinions, views or commentary of any testimonials on <?php echo $options["website"] ?> - the opinions are strictly
    the
...
<p><?php echo $options["company"] ?> may use software programs to create summary statistics, which are used for such
    purposes
...
<p>For site security purposes and to ensure that this service remains available to all
    users, <?php echo $options["company"] ?>
    uses software programs to monitor network traffic to identify unauthorized attempts to upload or change
    information, or otherwise cause damage.</p>

<h3>Disclaimer and Limitation of Liability</h3>

<p><?php echo $options["company"] ?> makes no representations, warranties, or assurances as to the accuracy, currency or
...
<p>All the materials on this site are provided "as is" without any express or implied warranty of any kind, including
    ...
    event shall <?php echo $options["company"] ?> or its agents or associates be liable for any damages whatsoever
    (including,
    without limitation, damages for loss of profits, business interruption, loss of information, injury or death)
    arising out of the use of or inability to use the materials, even if <?php echo $options["company"] ?> has been
    advised of
...
<p>If you have any questions regarding this policy, or your dealings with our website, please contact us
    here: <?php echo $options["contact"] ?></p>

<p><?php echo $options["company"] ?>
